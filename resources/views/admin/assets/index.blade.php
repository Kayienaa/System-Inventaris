...
<div class="max-w-7xl mx-auto px-6 py-8" x-data="{ deleteModalOpen: false, deleteUrl: '', deleteAssetName: '', deleteAssetCode: '' }">
    ...
    {{-- Flash Notifications --}}
    @if (session('success'))
        <div class="mb-6 flex items-center justify-between p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-800 dark:text-emerald-300 text-sm shadow-sm animate-fade-in">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/60 flex items-center justify-center shrink-0 text-emerald-600 dark:text-emerald-400">
    ...
    @if (session('error'))
        <div class="mb-6 flex items-center justify-between p-4 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800/60 text-rose-800 dark:text-rose-300 text-sm shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-rose-100 dark:bg-rose-900/60 flex items-center justify-center shrink-0 text-rose-600 dark:text-rose-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button type="button" @click="$el.parentElement.remove()" class="text-rose-500 hover:text-rose-700 dark:hover:text-rose-300">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
    @endif
    ...
    <div class="bg-white dark:bg-[#131B2A] rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800 p-5 mb-6">
        <form method="GET" action="{{ route('admin.assets.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            ...
            <div class="md:col-span-5">
                <label for="search" class="block text-xs font-semibold text-stone-700 dark:text-stone-300 mb-1.5">
                ...
                <div class="relative w-full">
                    ...
                    <input
                        ...
                        class="pl-10 pr-4 h-11 w-full rounded-xl border border-stone-300 dark:border-stone-700 bg-stone-50 dark:bg-[#0B0F17] text-stone-900 dark:text-stone-100 placeholder-stone-400 dark:placeholder-stone-500 focus:border-[#6F4E37] dark:focus:border-amber-500 focus:ring-1 focus:ring-[#6F4E37] text-sm"
                    >
                </div>
            </div>
            ...
            <div class="md:col-span-3">
                <label for="category" class="block text-xs font-semibold text-stone-700 dark:text-stone-300 mb-1.5">
                    Kategori Aset
                </label>
                <select
                    id="category"
                    name="category"
                    class="h-11 w-full px-3 text-sm rounded-xl border border-stone-300 dark:border-stone-700 bg-stone-50 dark:bg-[#0B0F17] text-stone-900 dark:text-stone-100 focus:border-[#6F4E37] dark:focus:border-amber-500 focus:ring-1 focus:ring-[#6F4E37] shadow-sm"
                >
            ...
            <div class="md:col-span-2">
                <label for="availability_status" class="block text-xs font-semibold text-stone-700 dark:text-stone-300 mb-1.5">
                    Status Ketersediaan
                </label>
                <select
                    ...
                    @forelse ($assets as $asset)
                        ...
                        <tr class="border-b border-stone-100 dark:border-stone-800/80 hover:bg-stone-50/50 dark:hover:bg-cyan-500/5 transition-colors">
                            ...
                            <td class="py-3 px-4 font-mono">
                                <span class="px-2 py-0.5 rounded bg-stone-100 dark:bg-stone-800/90 text-stone-700 dark:text-stone-300 font-semibold text-[11px] border border-stone-200 dark:border-stone-700">
                                    {{ $asset->asset_code }}
                                </span>
                                <p class="text-stone-400 dark:text-stone-500 text-[11px] mt-1">
                            ...
                            {{-- Kategori --}}
                            <td class="py-3 px-4">
                                <span class="px-2.5 py-1 rounded-full text-[11px] font-semibold bg-amber-50 text-[#6F4E37] border border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-800/60">
                                    {{ $asset->category->name ?? '-' }}
                                </span>
                            </td>
                            ...
                            <td class="py-3 px-4">
                                @if ($condVal === 'baik')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-800/50">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                        Baik
                                    </span>
                                @elseif ($condVal === 'rusak_ringan')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-800/50">
                                        <span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
                                        Rusak Ringan
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-700 border border-rose-200 dark:bg-rose-950/40 dark:text-rose-300 dark:border-rose-800/50">
                            ...
                            <td class="py-3 px-4">
                                @if ($statusVal === 'tersedia')
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-800/80">
                                        Tersedia
                                    </span>
                                @elseif ($statusVal === 'dipinjam')
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-blue-100 text-blue-800 border border-blue-300 dark:bg-blue-950/60 dark:text-blue-300 dark:border-blue-800/80">
                                        Dipinjam
                                    </span>
                                @elseif ($statusVal === 'perbaikan')
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-rose-100 text-rose-800 border border-rose-300 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-800/80">
                                        Perbaikan
                                    </span>
                                @elseif ($statusVal === 'dipesan')
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-100 text-amber-800 border border-amber-300 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-800/80">
                                        Dipesan
                                    </span>
                                @else
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-stone-100 text-stone-700 border border-stone-300 dark:bg-stone-800 dark:text-stone-300 dark:border-stone-700">
                                        Tidak Tersedia
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="py-3 px-4">
                                @if ($asset->activeBorrowing)
                                    <div>
                                        <p class="font-semibold text-stone-800 dark:text-stone-200">
                                ...
                                @else
                                    <span class="text-stone-400 dark:text-stone-500 text-xs italic">-</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-12 text-center">
                                <div class="w-16 h-16 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-800/40 flex items-center justify-center text-[#6F4E37] dark:text-neon-glowamber mx-auto mb-3">
                                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/>
                                    </svg>
                                </div>
                                <p class="text-sm font-bold text-stone-700 dark:text-stone-200">Tidak ada aset inventaris yang ditemukan</p>
                                <p class="text-xs text-stone-400 dark:text-stone-400 mt-1 max-w-sm mx-auto">
    ...
    >
        ...
        >
            ...
            <div class="my-4 p-3 bg-stone-50 dark:bg-[#0B0F17] rounded-xl border border-stone-200 dark:border-stone-800 text-xs">
                <p class="font-bold text-stone-800 dark:text-stone-200" x-text="deleteAssetName"></p>
                <p class="text-stone-500 dark:text-stone-400 font-mono mt-0.5">Kode: <span x-text="deleteAssetCode"></span></p>
            </div>

            <p class="text-[11px] text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 p-2.5 rounded-lg border border-amber-200 dark:border-amber-800/60">
