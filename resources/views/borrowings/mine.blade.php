...
@section('title', 'Peminjaman Saya | SITEFA')
...
<div class="max-w-5xl mx-auto px-6 py-8" x-data="mineBorrowingsHandler()">
    ...
    @if (session('error'))
        <div class="mb-6 rounded-2xl bg-rose-50/90 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-500/30 p-4 text-sm font-medium text-rose-800 dark:text-rose-300 flex items-center gap-3 shadow-sm">
                        ...
                        <div>
                            ...
                            <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-stone-500 dark:text-stone-400">
                                @if ($borrowing->requested_at)
                                    <span>Diajukan: {{ $borrowing->requested_at->format('d M Y, H:i') }}</span>
                                @endif
...
<script>
    function mineBorrowingsHandler() {
        return {
            ...
            isCheckoutCameraOpen: false,
            checkoutCapturedPhoto: null,
            checkoutMediaStream: null,
            checkoutFacingMode: 'environment',

            // --- CHECKOUT ACTIONS ---
            openCheckoutModal(borrowingId, name, code) {
                this.activeBorrowingId = borrowingId;
                this.activeAssetName = name;
                this.activeAssetCode = code;
                this.checkoutCapturedPhoto = null;
                this.checkoutModalOpen = true;
                this.openCheckoutCamera();
            },

            closeCheckoutModal() {
                this.closeCheckoutCamera();
                this.checkoutModalOpen = false;
            },

            async openCheckoutCamera() {
                try {
                    if (this.checkoutMediaStream) {
                        this.closeCheckoutCamera();
                    }
                    this.checkoutMediaStream = await navigator.mediaDevices.getUserMedia({
                        video: {
                            facingMode: this.checkoutFacingMode,
                            width: { ideal: 1280 },
                            height: { ideal: 720 }
                        },
                        audio: false
                    });
                    this.$refs.checkoutVideo.srcObject = this.checkoutMediaStream;
                    this.isCheckoutCameraOpen = true;
                    this.checkoutCapturedPhoto = null;
                } catch (err) {
                    console.error("Checkout camera access error:", err);
                }
            },
            ...
            takeCheckoutSnapshot() {
                ...
                this.applyWatermark(canvas, "{{ auth()->user()->name }}", "Serah Terima Pinjam");

                this.checkoutCapturedPhoto = canvas.toDataURL('image/jpeg', 0.68);
                this.closeCheckoutCamera();
            },

            retakeCheckoutSnapshot() {
                this.checkoutCapturedPhoto = null;
                this.openCheckoutCamera();
            },

            switchCheckoutCamera() {
                this.checkoutFacingMode = this.checkoutFacingMode === 'user' ? 'environment' : 'user';
                this.openCheckoutCamera();
            },
            ...
            onCheckoutSubmit(e) {
                if (!this.checkoutCapturedPhoto) {
                    e.preventDefault();
                    alert('Silakan ambil foto bukti fisik serah terima barang via kamera real-time terlebih dahulu.');
                    return;
                }
                this.closeCheckoutCamera();
            },
            ...
            applyWatermark(canvas, userName, typeLabel) {
                ...
                const textStr = `${typeLabel}: ${userName} | ${timestampStr}`;
