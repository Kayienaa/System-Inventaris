...
class UpdateAssetRequest extends FormRequest
{
    ...
    {
        $asset = $this->route('asset') ?? $this->asset;
        $assetId = $asset instanceof Asset ? $asset->id : $asset;

        return [
            'asset_category_id' => ['required', 'exists:asset_categories,id'],
            'asset_code' => ['required', 'string', Rule::unique('assets', 'asset_code')->ignore($assetId), 'max:50'],
            'serial_number' => ['required', 'string', Rule::unique('assets', 'serial_number')->ignore($assetId), 'max:100'],
