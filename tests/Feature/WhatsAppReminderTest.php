...
class WhatsAppReminderTest extends TestCase
{
    ...
    public function test_borrowing_show_page_displays_whatsapp_button(): void
    {
        $admin = User::factory()->create();
        $admin->assignRole('admin');
    ...
    public function test_borrowing_show_page_shows_disabled_whatsapp_button_when_phone_is_missing(): void
    {
        $admin = User::factory()->create();
        $admin->assignRole('admin');

        $siswa = User::factory()->create(['name' => 'Doni Tanpa HP']);
        $siswa->assignRole('siswa');
        SiswaProfile::create([
            'user_id' => $siswa->id,
            'nis' => '77665',
